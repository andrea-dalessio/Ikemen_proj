...
func processPlayerInput(playerIdx int, move string, btn string, facing float32) {
	...
	if strings.Contains(move, "up") || strings.Contains(move, "U") {
		up = true
	}
	if strings.Contains(move, "down") || strings.Contains(move, "D") {
		down = true
	}

	// Nota: controlliamo "left" minuscolo per evitare confusione con "L" di Player 2 tasto L se mai ci fosse ambiguità
	if strings.Contains(move, "left") {
		left = true
	}
	if strings.Contains(move, "right") {
		right = true
	}

	// Gestione codici brevi assoluti "L" e "R" (solo se stringa esatta o parte di diagonale nota)
	if strings.Contains(move, "L") && !strings.Contains(move, "Left") {
		left = true
	}
	if strings.Contains(move, "R") && !strings.Contains(move, "Right") {
		right = true
	}
